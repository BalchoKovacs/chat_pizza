...
const DATABASE_NAME = process.env.SQL_DATABASE || 'chat_hipo'
...
getTrigger = (number= '',callback) => connection.query(
    `SELECT * FROM ${DATABASE_NAME}.messages WHERE number = '${number}' and `+"`trigger`"+`!= 'null' ORDER BY date DESC LIMIT 1`,
    (error, results
        ) => {
    const [response] = results;
    const value = response?.trigger || null
    callback(value)
});
...
saveMessageMysql = ( message, trigger, number ) => new Promise((resolve,reejct) => {
    try {
        connection.query(
        `INSERT INTO ${DATABASE_NAME}.messages  `+"( `message`, `trigger`, `number`)"+` VALUES ('${message}','${trigger}','${number}')` , (error, results) => {
            if(error) {
